...
def build_app():
    ...
    parsed ``sys.argv`` itself via `argparse` (which accepts no
    arguments of its own), so there is never a leftover Qt-specific flag
    (e.g. ``-style``) for `QApplication` to consume.
    """
...
def build_window():
    """Construct the main window (panels only; unwired -- see :class:`~nickel_focus.scripts.focus_gui.NickelFocusGUI`)."""
    return MainWindow()
