...
class QtLogHandler(QtCore.QObject, logging.Handler):
    ...
    writing them anywhere itself.  Log calls can happen on a background thread,
    so this handler never touches a widget directly -- Qt automatically queues a
    :class:`PySide6.QtCore.Signal` emission onto the receiving slot's own
    thread, which is what keeps this safe.
    ...
    record_logged : :class:`PySide6.QtCore.Signal`
        Emitted with one formatted line of text per log record accepted by this
        handler (i.e., past its level filter).
    """
