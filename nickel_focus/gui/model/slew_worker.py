...
:meth:`~nickel_focus.slew.NickelTelescopePointing.slew_to` call off the GUI thread.

See :class:`~nickel_focus.gui.model.focus_worker.FocusWorker`'s module docstring: every
hardware call here is blocking, and
:meth:`~nickel_focus.slew.NickelTelescopePointing.slew_to` can wait up to five
minutes for the telescope to reach its target, so running it directly on
the GUI's event-loop thread would freeze the whole interface.
"""
...
class SlewWorker(QtCore.QThread):
    """
    Drive one :meth:`~nickel_focus.slew.NickelTelescopePointing.slew_to` call on a
    background thread.

    A new :class:`~nickel_focus.gui.model.slew_worker.SlewWorker` is expected to be
    created for each slew,
    matching :class:`PySide6.QtCore.QThread`'s own expectation that
    ...
    telescope : :class:`~nickel_focus.slew.NickelTelescopePointing`
        ...
        class only calls :meth:`~nickel_focus.slew.NickelTelescopePointing.slew_to` on it.
    ra
    ...
    slewFailed : :class:`PySide6.QtCore.Signal`
        ...
        :meth:`~nickel_focus.slew.NickelTelescopePointing.slew_to` raises -- movement
    ...
    def run(self):
        ...
        for why emitting signals from here is still safe to connect to
        GUI-thread slots.
        """
        try:
