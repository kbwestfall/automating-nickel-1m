...
class MainWindow(QtWidgets.QMainWindow):
    ...
    together is
    :class:`~nickel_focus.gui.controller.Controller`'s job, not this class's.
    ...
    saved on close, via :class:`PySide6.QtCore.QSettings`. Deliberately
    not saved on every change -- these are convenience defaults for next
    ...
    def closeEvent(self, event):
        ...
        a :class:`PySide6.QtWidgets.QWidget` *event handler* rather than
        connecting to a signal (contrast
        :class:`~nickel_focus.gui.controller.Controller`,
        which is all signal/slot connections) -- Qt calls this method itself
        whenever the window is about to close, however that was triggered
        (the close button, :meth:`PySide6.QtWidgets.QWidget.close`, or
        quitting the application), so overriding it is the standard way to
        run code at that specific moment.
        """
        self._save_settings()
    ...
    def showEvent(self, event):
        ...
        ``central``'s geometry doesn't reflect the
        :meth:`~nickel_focus.gui.views.main_window.MainWindow._size_to_screen`
        resize until the window is genuinely shown -- reading it any earlier
        in :meth:`~nickel_focus.gui.views.main_window.MainWindow.__init__`
        (even after a plain
        :meth:`PySide6.QtWidgets.QWidget.resize`, even after spinning the
        event loop with no :meth:`PySide6.QtWidgets.QWidget.show`) still
        returns a stale default, so this can't be done any sooner than here.
        Guarded to run once -- later show events (e.g. un-minimizing) must
        never undo a splitter drag the user made in between.
        """
    ...
    def _settings(self):
        """The :class:`PySide6.QtCore.QSettings` store this window persists to/restores from."""
    ...
    def _load_settings(self):
        ...
        *is* the opt-in state, rather than a separately-persisted flag
        ...
        Passing that current value as
        :meth:`PySide6.QtCore.QSettings.value`'s ``defaultValue``
